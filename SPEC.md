...
## 8. Module 5 — Public Masjid Registration Request

Any organisation can request to bring their masjid onto the platform without contacting a super admin first. The flow is:

1. **Landing page form** — embedded below the Sign In button on the public homepage. Fields: Masjid Name, Full Address, Contact Email, Contact Phone (all mandatory). A hidden honeypot field silently rejects bot submissions.

2. **Submission endpoint** (`POST /api/public/masjid-applications`) — no authentication required. Protections applied server-side:
   - IP-based rate limit: max 3 submission attempts per IP per 24 hours (hashed IP stored in `masjid_applications.ip_hash`).
   - Email uniqueness: one submission per email address enforced by a DB unique index.
   - Duplicate submissions return a clear message; flood attempts return HTTP 429.

3. **Storage** — saved to the `masjid_applications` table with `status = 'pending'`. Table columns: `id`, `created_at`, `name`, `address`, `email`, `phone`, `status` (`pending` / `approved` / `rejected`), `reviewed_at`, `notes`, `ip_hash`. RLS: super admin full access; all inserts performed via service role.

4. **Super admin notification** — after successful insert, an email is sent to `SUPER_ADMIN_EMAIL` (env var) containing all submitted fields and a direct link to `/superadmin/applications`. Falls back to `RESEND_FROM` if `SUPER_ADMIN_EMAIL` is not set. If neither is set, logs to console.

5. **Super admin Applications page** (`/superadmin/applications`) — lists all applications in a table with filter tabs: All / Pending / Approved / Rejected. Pending rows show two actions:
   - **Create Masjid** → navigates to `/superadmin/masjids/new?applicationId={id}`. The New Masjid form pre-fills Name, Address, Phone from the application and shows a blue info bar with the applicant's contact email for reference. On successful masjid creation the application is automatically marked `approved`.
   - **Reject** → inline confirm step; marks the application `rejected` via `PATCH /api/superadmin/applications/{id}`.

6. **Dashboard card** — the super admin dashboard gains a fourth stats card "Pending Applications" showing the live count. It links to `/superadmin/applications?status=pending` and turns amber when count > 0.

7. **No self-service masjid creation** — the applicant receives only a confirmation message on screen. Credentials are generated by the super admin through the normal masjid creation flow, unchanged from Module 1.
...
## 10. Build Order (work in this sequence, commit after each step)
...
10. Public masjid registration request form + super admin approval flow
