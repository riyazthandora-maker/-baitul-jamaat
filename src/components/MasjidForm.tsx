...
export default function MasjidForm({ initialData, isNew = false }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      masjid_code: (fd.get("masjid_code") as string).toUpperCase(),
    ...
    startTransition(async () => {
      ...
      if (isNew && data.credentials) {
        // Redirect to credentials page with data in sessionStorage
        sessionStorage.setItem("bj_credentials", JSON.stringify(data.credentials));
        router.push(`/superadmin/masjids/${data.masjid.id}/credentials`);
      } else {
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="e.g. BJM"
              maxLength={6}
              className="w-full rounded-lg border border-gray-300 px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-brand-green uppercase font-mono"
      ...
      <div className="bg-white rounded-xl border shadow-sm p-6 space-y-4">
        <h2 className="font-semibold text-gray-800 flex items-center gap-2">
          ...
          Location{" "}
          <span className="text-xs font-normal text-gray-400">(optional — click map to pin)</span>
        </h2>
